...
CONSTRAINTS = [
    "CREATE CONSTRAINT node_id IF NOT EXISTS FOR (n:NetworkNode) REQUIRE n.id IS UNIQUE",
    ...
    "CREATE INDEX node_domain IF NOT EXISTS FOR (n:NetworkNode) ON (n.domain)",
]
...
COMPUTE_NODES = [
    ...
    {"id": "HOST-DC1-03", "name": "Compute Host DC1 03", "type": "PHYSICAL_HOST",
     ...
     "cpu_cores": 48,    "ram_gb": 384,   "rack": "DC1-ROW-A-R03"},
    {"id": "HOST-DC2-01", "name": "Compute Host DC2 01", "type": "PHYSICAL_HOST",
     ...
     "cpu_cores": 64,    "ram_gb": 512,   "rack": "DC2-ROW-A-R01"},
    {"id": "HOST-DC2-02", "name": "Compute Host DC2 02", "type": "PHYSICAL_HOST",
     ...
     "cpu_cores": 48,    "ram_gb": 384,   "rack": "DC2-ROW-A-R02"},
]
...
SLICES = [
    {"id": "SLICE-EMBB-01", "name": "eMBB Slice West Region",
     "type": "eMBB",     "region": "West",
     "sla_latency_ms": 20, "sla_throughput_mbps": 100,
     "customer": "Consumer-Broadband"},
    {"id": "SLICE-URLLC-01", "name": "uRLLC Slice Industry",
     "type": "uRLLC",    "region": "West",
     "sla_latency_ms": 1,  "sla_throughput_mbps": 10,
     "customer": "Industrial-IoT"},
    {"id": "SLICE-EMBB-02", "name": "eMBB Slice South Region",
     "type": "eMBB",     "region": "South",
     "sla_latency_ms": 20, "sla_throughput_mbps": 100,
     "customer": "Consumer-Broadband"},
    {"id": "SLICE-MMTC-01", "name": "mMTC Slice IoT",
     "type": "mMTC",     "region": "West",
     "sla_latency_ms": 100,"sla_throughput_mbps": 1,
     "customer": "Smart-City-IoT"},
]
...
ALARMS = [
    ...
    # ── Scenario 2: Compute host failure → VNF down → Core NF unavailable ──
    {
        "id": "ALM-CMP-001",
    ...
    # ── A cleared alarm to show lifecycle ──────────────────────────────────
    {
        "id": "ALM-OPT-003",
...
PROPAGATION_EDGES = [
    ("ALM-OPT-001", "ALM-OPT-002"),  # Fiber cut → Amplifier fault
    ("ALM-OPT-001", "ALM-IP-001"),   # Fiber cut → IP link down
    ("ALM-IP-001",  "ALM-IP-002"),   # IP link down → BGP down
    ("ALM-IP-001",  "ALM-RAN-001"),  # IP link down → RAN cell OOS
    ("ALM-CMP-001", "ALM-CMP-002"),  # Host down → UPF crash
]
...
def create_optical_nodes(session):
    ...
    for n in OPTICAL_NODES:
        session.run("""
            MERGE (node:NetworkNode {id: $id})
            SET node += $props
            SET node:OpticalNode
        """, id=n["id"], props=n)
...
def create_compute_nodes(session):
    ...
    for v in VNFS:
        session.run("""
            ...
            SET vnf += $props
            WITH vnf
            MATCH (h:NetworkNode {id: $host_id})
            MERGE (h)-[:HOSTS]->(vnf)
...
def create_topology_edges(session):
    ...
    # Optical fiber spans (ROADM ↔ ROADM)
    fiber_spans = [
    ...
    for src, dst, span_id, name, km in fiber_spans:
        session.run("""
            MATCH (a:NetworkNode {id: $src})
            MATCH (b:NetworkNode {id: $dst})
    ...
    session.run("""
        MATCH (a:NetworkNode {id: 'ROADM-MUM-01'})
        MATCH (amp:NetworkNode {id: 'AMP-MUM-CHN-01'})
        MATCH (b:NetworkNode {id: 'ROADM-CHN-01'})
    ...
    session.run("""
        MATCH (otn:NetworkNode {id: 'OTN-MUM-01'})
        MATCH (roadm:NetworkNode {id: 'ROADM-MUM-01'})
        MERGE (otn)-[:CONNECTED_TO {type: 'client_port'}]->(roadm)
    """)
    session.run("""
        MATCH (otn:NetworkNode {id: 'OTN-CHN-01'})
        MATCH (roadm:NetworkNode {id: 'ROADM-CHN-01'})
        MERGE (otn)-[:CONNECTED_TO {type: 'client_port'}]->(roadm)
    """)
    ...
    for roadm, rtr, circuit_id, cap in optical_to_ip:
        session.run("""
            MATCH (o:NetworkNode {id: $roadm})
            MATCH (r:NetworkNode {id: $rtr})
    ...
    session.run("""
        MATCH (sw:NetworkNode {id: 'SW-AGG-DC1-01'})
        MATCH (h:NetworkNode) WHERE h.id IN ['HOST-DC1-01','HOST-DC1-02','HOST-DC1-03']
        MERGE (sw)-[:CONNECTED_TO {type: 'server_uplink', speed_gbps: 25}]->(h)
    """)
    session.run("""
        MATCH (sw:NetworkNode {id: 'SW-AGG-DC2-01'})
        MATCH (h:NetworkNode) WHERE h.id IN ['HOST-DC2-01','HOST-DC2-02']
        MERGE (sw)-[:CONNECTED_TO {type: 'server_uplink', speed_gbps: 25}]->(h)
    """)
    session.run("""
        MATCH (rtr:NetworkNode {id: 'RTR-P-CORE-01'})
        MATCH (sw:NetworkNode) WHERE sw.id IN ['SW-AGG-DC1-01','SW-AGG-DC1-02']
        MERGE (rtr)-[:CONNECTED_TO {type: 'dc_uplink', speed_gbps: 100}]->(sw)
    """)
    session.run("""
        MATCH (rtr:NetworkNode {id: 'RTR-P-CORE-02'})
        MATCH (sw:NetworkNode) WHERE sw.id IN ['SW-AGG-DC2-01']
        MERGE (rtr)-[:CONNECTED_TO {type: 'dc_uplink', speed_gbps: 100}]->(sw)
    """)

    # RAN backhaul — gNB to PE router (N2/N3 interfaces)
    ran_backhaul = [
    ...
    for gnb, rtr in ran_backhaul:
        session.run("""
            MATCH (g:NetworkNode {id: $gnb})
            MATCH (r:NetworkNode {id: $rtr})
            MERGE (g)-[:BACKHAUL_VIA {interfaces: 'N2,N3', speed_gbps: 10}]->(r)
        """, gnb=gnb, rtr=rtr)

    # gNB to core VNFs (N2 to AMF, N3 to UPF)
    session.run("""
        MATCH (g:NetworkNode) WHERE g.type = 'gNB'
        MATCH (amf:VNF {type: 'AMF'})
        MERGE (g)-[:N2_INTERFACE]->(amf)
    """)
    session.run("""
        MATCH (g:NetworkNode) WHERE g.type = 'gNB' AND g.region = 'West'
    ...
    service_deps = [
        ("SVC-BROADBAND-WEST",  ["RTR-PE-MUM-01", "RTR-P-CORE-01", "gNB-MUM-SITE-A01",
                                  "gNB-MUM-SITE-A02", "ROADM-MUM-01"]),
        ("SVC-VoNR-WEST",       ["RTR-PE-MUM-01", "RTR-P-CORE-01", "gNB-MUM-SITE-A01"]),
        ("SVC-BROADBAND-SOUTH", ["RTR-PE-CHN-01", "RTR-P-CORE-02", "gNB-CHN-SITE-A01",
                                  "ROADM-CHN-01"]),
    ...
    for svc_id, node_ids in service_deps:
        for node_id in node_ids:
            session.run("""
                ...
                MATCH (n:NetworkNode {id: $node_id})
...
def create_alarms(session):
    ...
    for a in ALARMS:
        ...
        # TRIGGERED_ON edge → network node
        session.run("""
            MATCH (alarm:Alarm {id: $alarm_id})
            MATCH (node:NetworkNode {id: $node_id})
...
def print_summary(session):
    ...
    """)
    for row in results:
        print(f"  {str(row['label']):30s} {row['count']:>5}")
    print()
    results = session.run("""
...
def main():
    ...
    print("  MATCH (n) RETURN n   (to see the full graph)")
    print("  MATCH p=(a:Alarm {isRootCause:true})-[:PROPAGATED_TO*]->(b:Alarm)")
    print("  RETURN p             (to see alarm propagation chains)")
