...
def set_geographic_coordinates(session):
    """Add lat/lon coordinates to all nodes for Neovis geographic layout."""
    city_coords = {
    ...
    # Small per-node offsets so nodes in the same city don't overlap
    import random
    random.seed(42)
    for city, coords in city_coords.items():
        session.run("""
            MATCH (n) WHERE n.city = $city
            SET n.latitude  = $lat + (rand() - 0.5) * 0.08,
                n.longitude = $lon + (rand() - 0.5) * 0.08
        """, city=city, lat=coords["lat"], lon=coords["lon"])
    print("  Geographic coordinates set on all nodes.")
...
def main():
    ...
    with driver.session() as session:
        ...
        set_geographic_coordinates(session)
